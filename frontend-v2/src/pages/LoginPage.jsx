...
export default function LoginPage() {
  ...
  const handleGetOTPValue = async (e) => {
    try {
      ...
      alert("Get OTP successfully!");
      setIsOtpModalOpen(true); 
      setCountdown(300); // Reset countdown to 5 minutes when OTP is requested
    } catch (err) {
      alert("Get OTP failed. Please try again.");
    }
  };
  ...
  useEffect(() => {
    ...
    if (isOtpModalOpen && countdown > 0) {
      ...
      setIsOtpModalOpen(false); // Close OTP modal when timer expires
      // setRedirect(true);
      navigate("/login");
    }
    return () => clearInterval(timer); // Clean up interval when component unmounts or countdown reaches 0
  ...
  return (
    <div className="pt-5 flex flex-col min-h-screen">
      ...
      <div className="mt-4 grow flex items-center justify-center">
        <div className="mb-32 border p-5 border-gray-300 rounded-xl">
          <h1 className="text-4xl font-bold text-center mb-4 text-gray-900">Login</h1>
          <form className="max-w-md mx-auto" onSubmit={handleLoginSubmit}>
            <InputField
              label="Your email"
              id="email"
            ...
            <button className="login-button">Login</button>
            <button
              className="mt-6 text-left text-sm text-gray-900 hover:text-teal-500 hover:underline cursor-pointer"
            ...
            <p className="text-center py-2 text-gray-900">
              Don't have an account yet?{" "}
              <Link className="font-semibold underline" to="/register">Register</Link>
            </p>
          </form>
        </div>
      </div>
          ...
          <div className="modal-content">
            ...
            <div className="flex justify-center gap-4 mt-4">
              <button
                onClick={async () => {
                  try {
                    const { data } = await axios.post("/user/otp/verify", { email: formData.email, otp, context });
                    console.log(data);
                    localStorage.setItem("token", data.accessToken);
                    setUser(data);
                    alert("OTP verified successfully!");
  ...
  return (
    <header className="flex justify-between px-10 mb-7">
      <Link to="/" className="flex items-center gap-1 text-primary md:pl-2 lg:pl-5">
        ...
        <span className="font-bold text-2xl text-primary hidden md:flex">StayEasy</span>
      </Link>
    </header>
  );
}
